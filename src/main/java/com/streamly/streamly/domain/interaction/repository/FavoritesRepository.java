...
public interface FavoritesRepository extends JpaRepository<Favorites,Long> {
    ...
    @Query(value = "SELECT f FROM Favorites f JOIN FETCH f.video v JOIN FETCH v.uploader WHERE f.user = :user",
           countQuery = "SELECT COUNT(f) FROM Favorites f WHERE f.user = :user")
